...
public class CPABE {
    ...
    public static void setup(String pairingParametersFileName, String pkFileName, String mskFileName) {
        ...
        Element f = g.powZn(bp.getZr().newElement(1).div(beta)).getImmutable();
        ...
        pkProp.setProperty("g", Base64.getEncoder().withoutPadding().encodeToString(g_alpha.toBytes()));
        ...
        pkProp.setProperty("f", Base64.getEncoder().withoutPadding().encodeToString(f.toBytes()));

        storePropToFile(mskProp, mskFileName);
        storePropToFile(pkProp, pkFileName);
    }

    public static void keygen(String pairingParametersFileName, String[] userAttList, String pkFileName, String mskFileName, String skFileName ) {
        Pairing bp = PairingFactory.getPairing(pairingParametersFileName);

        Properties pkProp = loadPropFromFile(pkFileName);
        String gString = pkProp.getProperty("g");
        Element g = bp.getG1().newElementFromBytes(Base64.getDecoder().decode(gString));
        ...
        Element D = g_alpha.mul(g_r).div(g_beta).getImmutable();
        ...
        for (String att : userAttList) {
            byte[] idHash = Integer.toString(att.hashCode()).getBytes();
            Element H = bp.getG1().newElementFromHash(idHash, 0, idHash.length).getImmutable();
            Element r_j = bp.getZr().newRandomElement();
            Element D_j = g_r.mul(H.powZn(r_j));
            Element D_1 = g.powZn(r_j);

            skProp.setProperty("D_j"+att, Base64.getEncoder().withoutPadding().encodeToString(D_j.toBytes()));
            skProp.setProperty("D_1"+att, Base64.getEncoder().withoutPadding().encodeToString(D_1.toBytes()));
        }
    ...
    public static void encrypt(String pairingParametersFileName, Element message, Node[] accessTree,
        ...
        for (Node node:accessTree) {
            if (node.isLeaf()){
                ...
                byte[] idHash = Integer.toString(node.hashCode()).getBytes();
                Element Hi = bp.getG1().newElementFromHash(idHash, 0, idHash.length).getImmutable();
    ...
    public static Element decrypt(String pairingParametersFileName, Node[] accessTree, String ctFileName, String skFileName) {
        ...
        //恢复用户属性列表 int[]类型
        String[] userAttList = (String[]) Arrays.stream(userAttListString.substring(1, userAttListString.length()-1).split(",")).map(String::trim).toArray();
        ...
        for (Node node : accessTree) {
            if (node.isLeaf()) {
                ...
                if (Arrays.stream(userAttList).collect(Collectors.toList()).contains(node.att)){
    ...
    public static boolean nodeRecover(Node[] nodes, Node n, String[] atts, Pairing bp) {
        if (!n.isLeaf()) {
            ...
            if (Arrays.stream(atts).collect(Collectors.toList()).contains(n.att)){
                n.valid = true;
            }
        }
        return n.valid;
